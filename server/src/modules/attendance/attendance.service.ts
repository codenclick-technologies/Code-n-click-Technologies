...
import { CheckInDto, CheckOutDto, QueryAttendanceDto } from './dto/attendance.dto';
...
export class AttendanceService {
    ...
    async getMonthlyReport(userId: string, month: number, year: number) {
        ...
        const presentDays = records.filter((r: any) => r.status === AttendanceStatus.PRESENT).length;
        const absentDays = records.filter((r: any) => r.status === AttendanceStatus.ABSENT).length;
        const leaveDays = records.filter((r: any) => r.status === AttendanceStatus.LEAVE).length;
        const halfDays = records.filter((r: any) => r.status === AttendanceStatus.HALF_DAY).length;
